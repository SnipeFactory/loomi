...
## Memory-First Rule (Selective)

이 프로젝트는 Loomi 에피소딕 메모리를 통한 맥락 조회를 기본으로 한다. 단, 효율성을 위해 **의사결정 및 구현(Directive)**이 필요한 경우에만 전역 `Selective Memory Research Protocol`을 따라 검색을 수행한다.
